...
export default function CardContents({ title, cover_image, slug, category}) {

    // Funzione per mappare i colori alle categorie 🎨
    const getBadgeStyle = (cat) => {
        const cleanCat = cat?.replaceAll('-', ' ').toLowerCase();
        switch (cat?.toLowerCase()) {
            case "luoghi-da-visitare": return { backgroundColor: "#dc3545" }; // Rosso 🔴
            case "cosa-mangiare":      return { backgroundColor: "#ffc107", color: "#000" }; // Giallo 🟡
            case "itinerari":              return { backgroundColor: "#0d6efd" }; // Blu 🔵
